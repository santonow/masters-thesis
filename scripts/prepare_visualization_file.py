...
sources: Dict[str, str] = {
    k: v for k, v in snakemake.input.items() if k not in {"taxonomy", "trophic_groups"}
}


def read_graph(
    sources: Dict[str, str],
    otu_to_lineage: Dict[str, Tuple[str, ...]],
    taxa_to_trophic_group: Dict[str, Set[str]],
) -> nx.Graph:
    graph = nx.Graph()
    ...
    for head, tail, attrs in graph.edges(data=True):
        weights, signs = [], []
        for source, weight, sign in zip(
            attrs["sources"], attrs["weights"], attrs["signs"]
        ):
            if source == "consensus":
...
def collect_statistics(graph, all_trophic_groups: Set[str]):
    ...
    stats["pos"] = {node: c["+"] / nx.degree(graph, node) for node, c in signs.items()}
    stats["neg"] = {node: c["-"] / nx.degree(graph, node) for node, c in signs.items()}
    stats["weights"] = {node: np.mean(ws) for node, ws in weights.items()}
