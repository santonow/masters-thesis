    ...
    def to_csv(
        ...
        with self.create_writer(gzipped)(filepath) as writer:
            ...
            if min_tot_abundance is not None:
                chosen_ids &= {
                    _id
                    for _id in tbl.ids(axis="observation")
                    if sum(tbl.data(_id, axis="observation")) > min_tot_abundance
                }
            filtered_by_total_abundance = (
                len(set(self.table.ids(axis="observation")) - chosen_ids)
                - filtered_by_tax
            )
            ...
            if relative_abundance is not None:
                ...
                chosen_ids &= {
                    _id
                    for _id in tbl.ids(axis="observation")
                    if sum(tbl.data(_id, axis="observation")) / tbl_sum
                    >= relative_abundance
                }
            filtered_by_relative_abundance = (
                len(set(self.table.ids(axis="observation")) - chosen_ids)
                - filtered_by_tax
                - filtered_by_total_abundance
            )
            if sample_threshold:
                ...
                chosen_ids &= {
                    _id
                    for _id in tbl.ids(axis="observation")
                    if list(tbl.data(_id, axis="observation")).count(0) / n_samples
                    < 1 - sample_threshold
                }
            filtered_by_zero_count = (
                len(set(self.table.ids(axis="observation")) - chosen_ids)
                - filtered_by_tax
                - filtered_by_total_abundance
                - filtered_by_relative_abundance
            ...
            filtered_table = tbl.filter(
                ids_to_keep=chosen_ids, invert=True, axis="observation", inplace=False
            )
            leftover_vector = ["leftover_vector"] + [
                x for x in filtered_table.sum(axis="sample")
            ]
            tbl = tbl.filter(ids_to_keep=chosen_ids, axis="observation", inplace=False)
            print(
