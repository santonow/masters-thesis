...
def determine_best_lineage(
    hsps: List[SearchIO.HSP], first_n: int = 10
) -> Optional[List[str]]:
    if len(hsps) < first_n:
        return None
    lineages = [
        get_lineage_from_description(hsp.hit_id)
        for hsp in sorted(hsps, key=lambda x: x.bitscore, reverse=True)[:first_n]
    ]
    i = 8
...
def prepare_tax_table(in_file, n_threads, out_file):
    ...
    for _ in range(threads):
        process = Process(target=reader, args=(in_queue, out_queue))
        process.start()
