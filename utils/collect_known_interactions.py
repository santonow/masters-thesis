...
for record in df.to_dict(orient='records'):
    lineage_left = [
        record[f'Taxonomic level {i}: org1'] for i in range(1, 4)
    ] + [record['Genus org1'], record['Species org1']]
    lineage_right = [
        record[f'Taxonomic level {i}: org2'] for i in range(1, 4)
    ] + [record['Genus org2'], record['Species org2']]
    ...
    if left_match is not None and right_match is not None:
        relations.add_edge(
            tuple(left_match), tuple(right_match), interaction=record['Interaction'],
            ecological_interaction=record['Ecological interaction'], habitat=record['Habitat']
        )

with open(snakemake.output[1], 'w') as handle:
    writer = csv.writer(handle, delimiter='\t')
    writer.writerow(['head', 'tail', 'interaction', 'ecological_interaction', 'habitat'])
    for head, tail, attrs in relations.edges(data=True):
        writer.writerow([
            json.dumps(head), json.dumps(tail), *[
                attrs[field_name] for field_name in ['interaction', 'ecological_interaction', 'habitat']
            ]
        ])
