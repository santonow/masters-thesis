...
class CoNet:
    ...
    def initial_correlation(self):
        ...
        for method, matrix in self.matrices_by_method.items():
            ...
            edges, left, right = self.select_edges(correlations, method, self.n_initial_edges)
            all_edges.append(edges)
            print(f'Selecting {self.n_initial_edges} initial edges, metric value < {left} and > {right}')
    ...
    def run(self):
        results = dict()
        for method, matrix in self.matrices_by_method.items():
            with timer(f'Computing p-values for method {method} using {self.n_jobs} processes...'):
                if self.n_jobs == 1:
                    pvalues, bs_samples = compute_pvals(matrix, self.n_iter, method, self.common_edges)
                else:
                    pvalues, bs_samples = self.compute_pvals_parallel(matrix=matrix, method=method)
                left_interval, right_interval = np.percentile(
                    np.array(bs_samples),
                    [100 * (1 - self.confidence_interval) / 2, 100 * (1 - (1 - self.confidence_interval) / 2)]
                )
        ...
        for method, result in results.items():
            ...
            for i in range(correlations.shape[0]):
                for j in range(correlations.shape[1]):
                    if i != j and i < j and ((i, j) in self.common_edges or (j, i) in self.common_edges):
                        if (
                                pvals[i, j] < self.p_value_threshold
